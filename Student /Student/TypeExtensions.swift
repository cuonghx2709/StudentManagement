...
extension UIView {
    ...
    @objc func setDoubleTapAction(_ action:((UIView)->Void)?){
    ...
    @objc func setDoubleTouchTapAction(_ action:((UIView)->Void)?){
        if let action = action {
            let gest = UITapGestureRecognizer(){ action(self) }
    ...
    @objc func requiresActionsToFail(_ view: UIView?){
    ...
    @objc var doubleTapGesture: UITapGestureRecognizer?{
    ...
    @objc var doubleTouchTapGesture: UITapGestureRecognizer?{
    ...
    @objc var pressGesture: UILongPressGestureRecognizer? {
    ...
    @objc var edgeGestures: [UIScreenEdgePanGestureRecognizer]?{
        return self.gestureRecognizers?.filter({ $0 is UIScreenEdgePanGestureRecognizer }) as? [UIScreenEdgePanGestureRecognizer]
    }
    
    @objc func addActivityIndicatorOverlay(_ completion:((_ remove: @escaping()->Void)->Void)?) {
    ...
    @objc func addActivityIndicatorWhiteBackground(_ completion:((_ remove: @escaping()->Void)->Void)?) {
        ...
        async_serial_main_wait(ser){ next in
            ...
            let indict = UIActivityIndicatorView(frame: CGRect(origin: CGPoint.zero, size: actView.bounds.size/7))
            indict.center = actView.center; indict.activityIndicatorViewStyle = UIActivityIndicatorViewStyle.whiteLarge
            indict.backgroundColor = .white
            actView .addSubview(indict)
            indict .startAnimating()
            var removeTask:(()->Void)? = { next() }
            self.smoothAddSubview(actView, duration: 0.5) { completion?({oneShot(&removeTask)}) }
    ...
    @objc func removeActivityIndicatorOverlay() {
        async_main {
    ...
    @objc func smoothAddSubview(_ view: UIView, duration: TimeInterval, completion: (()->Void)?) {
    ...
    @objc func smoothHide(duration: TimeInterval, completion: (()->Void)?) {
        
        UIView .animate(withDuration: duration, animations: { self.alpha = 0 }, completion: { b in completion?() })
    }
    
    @objc func smoothChangeAlpha(_ a: CGFloat, duration: TimeInterval, completion: (()->Void)?) {
...
extension UIVisualEffectView {
    ...
    @objc func darken(_ duration: Double) {
    ...
    @objc func lighten(_ duration: Double){
...
class CircleView: UIView {
    ...
    fileprivate class Action: NSObject, CAAction {
        @objc var pendingAnimation: CABasicAnimation?
        @objc var priorCornerRadius: CGFloat = 0
...
class Ring:CircleView
{
    ...
    @objc var backColor = UIColor.clear
    @objc var thickness = CGFloat(4)
...
extension UIGestureRecognizer {
    ...
    @objc convenience init(trailingClosure closure: @escaping (() -> ())) {
...
extension UIColor {
    ...
    @objc static func colorWithName(_ colorName:String)->UIColor{
